...
export const Refunds: React.FC = () => {
  ...
  const { data, isLoading, isError, refetch, error } = useQuery({
    queryKey: ['adminRefunds', page, searchTerm],
    queryFn: () => adminApi.getRefunds({ page, limit: 25, search: searchTerm }),
    staleTime: 1000 * 30,
  });
  ...
  if (isError) {
    return (
      <AdminLocalErrorState
        ...
        message={(error as any)?.message || 'Refund log could not be loaded.'}
        onRetry={() => refetch()}
      />
    );
  }
  ...
  const normalised = normaliseApiResponse(data, 'refunds');
  const refunds = normalised.data;
  ...
  const filtered = refunds.filter((r: any) =>
    ...
    r?.transactionReference?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Track and process customer refund requests.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div className="text-xs text-gray-500 font-medium">
          Showing {filtered.length} of {pagination.total} refunds
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm p-12 text-center text-gray-500">
          ...
          <p className="text-sm font-semibold text-gray-900">No refunds logged</p>
          <p className="text-xs text-gray-500 mt-0.5 font-normal">
            Customer refund claims will appear here for processing.
          </p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((r: any) => (
                  <tr key={r._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-mono text-xs text-gray-600">{r.transactionReference || '—'}</td>
                    <td className="px-6 py-4 text-xs text-gray-600">{r.bookingNumber || 'N/A'}</td>
                    <td className="px-6 py-4 text-xs text-gray-600">{r.customerEmail || '—'}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{r.providerName || 'Unknown provider'}</td>
                    <td className="px-6 py-4 font-bold text-red-600">{formatMoney(r.amount)}</td>
                    ...
                    <td className="px-6 py-4 text-xs text-gray-500">{formatDateSafe(r.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
